...
class CollectionScheduler:
    ...
    def _run_adaptive_collection(self) -> bool:
        ...
        try:
            ...
            logger.info(f"📅 스케줄 수집 범위: {start_date} ~ {end_date} (90일)")
    ...
    def _cleanup_expired_ips(self):
        ...
        try:
            ...
            with db_service.get_connection() as conn:
                ...
                # 2. 3개월(90일) 이상 지난 IP 비활성화 (단, 해제일이 미래인 경우 제외)
                cursor.execute(
                    """
                    UPDATE blacklist_ips
    ...
    def _collect_regtech_data(self, username: str, password: str, max_pages: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            if max_pages == 1:
                # 자동 수집: 90일 범위 (스케줄과 동일)
                start_date = (today - timedelta(days=90)).strftime("%Y-%m-%d")
                logger.info(f"🚀 REGTECH 스케줄 수집 (90일: {start_date} ~ {end_date})")
            else:
                # 수동 수집: 90일 범위
                start_date = (today - timedelta(days=90)).strftime("%Y-%m-%d")
