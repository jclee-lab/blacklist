...
class CollectorConfig:
    ...
    def _load_credentials_from_db(cls) -> None:
        """DB에서 인증정보 조회 (encrypted 컬럼 확인 후 복호화)"""
        ...
        try:
            ...
            # collection_credentials 테이블에서 인증정보 조회 (encrypted 컬럼 포함)
            cur.execute("""
            ...
            for row in cur.fetchall():
                ...
                username = row[1]
                password = row[2]
                # config = row[3]  # unused for now
                is_encrypted = row[4] if row[4] is not None else False
                ...
                if is_encrypted:
                    try:
                        ...
                        if not master_key:
                            logger.warning(f"CREDENTIAL_MASTER_KEY 없음 ({source})")
                        ...
                        logger.info(f"✅ DB 인증정보 복호화 성공: {source}")

                    except Exception as decrypt_error:
                        logger.warning(
                            f"복호화 실패 ({source}): {decrypt_error}"
                        )
                        # 복호화 실패 시 빈 값으로 설정 (잘못된 평문 사용 방지)
                        cls._credentials_cache[source] = {
                            "username": "",
                            "password": "",
                        }
                else:
                    # 평문 데이터 (encrypted=false)
                    cls._credentials_cache[source] = {
                        "username": username,
                        "password": password,
                    }
