...
class CollectorScheduler:
    ...
    def _initialize_collectors(self):
        """Initialize collectors metadata only (lazy initialization)"""
        try:
            # REGTECH collector metadata (no API connection yet)
            self.collectors['REGTECH'] = {
                'instance': None,  # Lazy init on first run
                'type': 'monitor',
                'last_run': None,
                'next_run': None,
                'run_count': 0,
            ...
            logger.info("✅ REGTECH collector metadata ready (lazy init)")

            # SECUDIUM collector metadata (no API connection yet)
            ...
            if credentials and credentials['enabled']:
                ...
                logger.info("✅ SECUDIUM collector metadata ready (lazy init)")
            else:
                logger.info("⚠️ SECUDIUM collector disabled or credentials not found")
    ...
    def _execute_collection(self, source_name: str) -> Dict[str, Any]:
        """Execute collection for a specific source (lazy initialization)"""
        ...
        try:
            logger.info(f"▶️ Executing {source_name} collection...")
            start_time = time.time()

            # Lazy initialization: Create instance only when needed
            instance = collector_info['instance']
            if instance is None:
                logger.info(f"🔄 Lazy initializing {source_name} collector instance...")

                if source_name == 'REGTECH':
                    # Get credentials from DB (fallback to env vars)
                    regtech_id, regtech_pw = CollectorConfig.get_regtech_credentials()
                    if not regtech_id or not regtech_pw:
                        return {'success': False, 'error': 'REGTECH credentials not configured'}

                    # Create REGTECH monitor instance
                    instance = REGTECHPolicyMonitor(self.config.to_dict())
                    collector_info['instance'] = instance
                    logger.info(f"✅ REGTECH collector instance created")
                    ...
                    if not secudium_id or not secudium_pw:
                        return {'success': False, 'error': 'SECUDIUM credentials not configured'}

                    # Create SECUDIUM API client
                    from api.secudium_api import SecudiumAPIClient
                    instance = SecudiumAPIClient(secudium_id, secudium_pw)
                    collector_info['instance'] = instance
                    logger.info(f"✅ SECUDIUM collector instance created")

                else:
                    return {'success': False, 'error': f'Unknown collector: {source_name}'}
                ...
                try:
                    cursor.execute("""
                        INSERT INTO blacklist_ips (
                            ...
                            removal_date, source, raw_data, is_active
                        ) VALUES (%s, %s, %s, %s, %s, %s, %s, true)
                        ON CONFLICT (ip_address, source)
                        DO UPDATE SET
