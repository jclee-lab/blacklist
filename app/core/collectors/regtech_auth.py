...
class RegtechAuth:
    ...
    def robust_login(self, session: requests.Session) -> bool:
        ...
        try:
            ...
            login_data = {"memberId": self.username, "memberPw": self.password}
            logger.info(f"Attempting login to URL: {login_url}")
            logger.info(f"Login data: memberId={self.username}")
            ...
            if response.status_code in [200, 302]:
                ...
                if session.cookies:
                    ...
                    self.cookie_auth_mode = True
                    logger.info(f"Saved {len(session.cookies)} cookies from login")
                return True
            else:
