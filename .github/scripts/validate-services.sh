...
echo "🧪 Validating services at $TARGET_URL"
...
# 2. API stats endpoint (tests database connectivity)
if test_endpoint "$TARGET_URL/api/stats" "API stats endpoint"; then
    API_PASSED=true
else
    API_PASSED=false
fi

# 3. Monitoring dashboard (tests Redis connectivity)
if test_endpoint "$TARGET_URL/api/monitoring/dashboard" "Monitoring dashboard"; then
    MONITOR_PASSED=true
else
...
echo "API Stats: $([ "$API_PASSED" = true ] && echo "✅ PASS" || echo "❌ FAIL")"  
echo "Monitoring: $([ "$MONITOR_PASSED" = true ] && echo "✅ PASS" || echo "❌ FAIL")"
...
if [ "$HEALTH_PASSED" = true ] && [ "$API_PASSED" = true ] && [ "$MONITOR_PASSED" = true ]; then
    ...
    echo ""
    echo "🚨 Service validation failed - system not ready"
