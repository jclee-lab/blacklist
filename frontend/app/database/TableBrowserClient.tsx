...
export default function TableBrowserClient() {
    ...
    queryFn: async () => {
      ...
      if (typeof rawData === 'object' && rawData !== null) {
        return Object.entries(rawData).map(([name, info]) => ({
          name,
          column_count: Array.isArray(info)
            ? info.length
            : ((info as { columns?: unknown[] })?.columns?.length ?? 0),
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const payload = await getDatabaseTableData(
        selectedTable,
  ...
  const totalPages =
    pagination.total_pages || (totalRows > 0 ? Math.ceil(totalRows / PAGE_SIZE) : 0);
          ...
          {isSchemaLoading ? (
            ...
            <div className="space-y-2 overflow-y-auto custom-scrollbar flex-1 pr-1">
              {tables.length > 0 ? (
                tables.map((table) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <FileJson
                        className={`h-4 w-4 flex-shrink-0 ${selectedTable === table.name ? 'text-blue-600' : 'text-gray-500'}`}
                      />
                      <div className="flex-1 min-w-0">
                        <p
                          className={`font-medium text-sm truncate ${selectedTable === table.name ? 'text-blue-900' : 'text-gray-700'}`}
                        >
                          {table.name}
                        </p>
                ...
                <div className="text-center py-8 text-gray-500 text-sm">테이블이 없습니다.</div>
              )}
            </div>
          )}
        </div>
